odoo.define('dcm_customization.FunOrgChart', function (require) {
"use strict";
...
var FieldOrgChart = AbstractField.extend({
    ...
    _getOrgData: function () {
        ...
        return this.dm.add(this._rpc({
            route: '/dcm/get_fun_sg_org_chart',
            params: {
                partner_id: this.recordData.id,
    ...
    _render: function () {
        if (!this.recordData.id) {
            return this.$el.html(QWeb.render("hr_org_chart_sg_fun", {
                managers: [],
                children: [],
            }));
        }
        ...
        return this._getOrgData().then(function (orgData) {
            ...
            self.$el.html(QWeb.render("hr_org_chart_sg_fun", orgData));
            self.$('[data-toggle="popover"]').each(function () {
                $(this).popover({
                    html: true,
                    title: function () {
                        var $title = $(QWeb.render('hr_orgchart_emp_popover_title_sg_fun', {
                            employee: {
                    ...
                    trigger: 'focus',
                    content: function () {
                        var $content = $(QWeb.render('hr_orgchart_emp_popover_content_sg_fun', {
                            employee: {
                                id: $(this).data('emp-id'),
                                name: $(this).data('emp-name'),
                    ...
                    template: QWeb.render('hr_orgchart_emp_popover_sg_fun', {}),
                });
            });
        });
    },
...
field_registry.add('sg_fun_org_chart', FieldOrgChart);
