odoo.define('dcm_customization.OrgChart', function (require) {
"use strict";
...
var FieldOrgChart = AbstractField.extend({
    ...
    _render: function () {
        if (!this.recordData.id) {
            return this.$el.html(QWeb.render("hr_org_chart_sg", {
                managers: [],
                children: [],
            }));
        }
        ...
        return this._getOrgData().then(function (orgData) {
            ...
            self.$el.html(QWeb.render("hr_org_chart_sg", orgData));
            self.$('[data-toggle="popover"]').each(function () {
                $(this).popover({
                    html: true,
                    title: function () {
                        var $title = $(QWeb.render('hr_orgchart_emp_popover_title_sg', {
                            employee: {
                    ...
                    trigger: 'focus',
                    content: function () {
                        var $content = $(QWeb.render('hr_orgchart_emp_popover_content_sg', {
                            employee: {
                                id: $(this).data('emp-id'),
                                name: $(this).data('emp-name'),
                    ...
                    template: QWeb.render('hr_orgchart_emp_popover_sg', {}),
                });
            });
        });
    },
...
field_registry.add('sg_org_chart', FieldOrgChart);
